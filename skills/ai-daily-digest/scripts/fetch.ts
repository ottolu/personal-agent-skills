#!/usr/bin/env bun
/**
 * fetch.ts — Pure RSS/Atom fetcher for daily-digest skill.
 * Zero AI dependency. Fetches feeds, filters by time, outputs JSON to stdout.
 *
 * Usage:
 *   npx -y bun fetch.ts --hours 48
 *   npx -y bun fetch.ts --hours 24 --output /tmp/articles.json
 */
...
function parseRSSItems(
  ...
  if (isAtom) {
    ...
    while ((m = entryPattern.exec(xml)) !== null) {
      ...
      let link = getAttrValue(entry, 'link[^>]*rel="alternate"', "href");
      if (!link) link = getAttrValue(entry, "link", "href");
      const pubDate =
...
async function fetchFeed(feed: (typeof RSS_FEEDS)[0]): Promise<Article[]> {
  try {
    ...
    const response = await fetch(feed.xmlUrl, {
      ...
      headers: {
        "User-Agent": "Daily-Digest/2.0 (RSS Reader)",
        Accept:
    ...
    if (!msg.includes("abort")) {
      console.error(`[fetch] x ${feed.name}: ${msg}`);
    } else {
      console.error(`[fetch] x ${feed.name}: timeout`);
    }
    return [];
  }
}
...
async function fetchAllFeeds(): Promise<Article[]> {
  ...
  for (let i = 0; i < RSS_FEEDS.length; i += FEED_CONCURRENCY) {
    ...
    const progress = Math.min(i + FEED_CONCURRENCY, RSS_FEEDS.length);
    console.error(
      `[fetch] ${progress}/${RSS_FEEDS.length} feeds (${ok} ok, ${fail} failed)`
    );
  }

  console.error(
    `[fetch] Done: ${all.length} articles from ${ok} feeds (${fail} failed)`
  );
  return all;
}
...
async function main() {
  ...
  console.error(`[fetch] Fetching 90 RSS feeds, filtering last ${hours}h...`);

  const articles = await fetchAllFeeds();
  const cutoff = new Date(Date.now() - hours * 60 * 60 * 1000);
  const recent = articles.filter((a) => {
    if (!a.pubDate) return false;
    return new Date(a.pubDate).getTime() > cutoff.getTime();
  });

  console.error(
    `[fetch] ${recent.length} articles within last ${hours}h (from ${articles.length} total)`
  );
  ...
  if (outputFile) {
    ...
    console.error(`[fetch] Written to ${outputFile}`);
  } else {
    console.log(output);
  }
}
